	...
	def __init__(self, data_file_name, data_dir='.data/'):
		...
		with open(data_path,encoding='utf-8') as csv_file:
			...
			for index, row in csv_reader.iterrows():
				q, a = row[colums[0]], row[colums[1]]
				#print(q, a)
				data_str = f"{q}: {a}{self.end_of_text_token}"
...
def save_model(model, name, tokenizer):
	"""
	Summary:
	...
	torch.save(model.state_dict(), f"{name}.bin")
	tokenizer.save_pretrained("./extract_finetune/")
	return
...
if __name__ == '__main__':
	...
	parser.add_argument('--model_name', default='extract_finetune', type=str, action='store', help='Name of the model file')
	parser.add_argument('--data_file', default='dataset/extract_data.csv', type=str, action='store', help='Name of the data file')
	parser.add_argument('--batch', type=int, default=32, action='store', help='Batch size')
	parser.add_argument('--learning_rate', default=1e-5, type=float, action='store', help='Learning rate for the model')
	parser.add_argument('--max_len', default=128, type=int, action='store', help='Maximum length of sequence')
	...
	save_model(model, MODEL_NAME, TOKENIZER)
