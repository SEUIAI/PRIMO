...
class PPOTrainer:
    ...
    def step(self, queries, responses, scores,device,tokenzier):
        """
        ...
        t = time.time()
        logprobs, ref_logprobs, values = self.batched_forward_pass(queries, responses,device,tokenzier)          # 拿到模型生成的tokens的log_prob、token_value
        ...
        for _ in range(self.ppo_params['ppo_epochs']):
            ...
            for i in range(bs):
                ...
                train_stats = self.train_minibatch(logprobs[idx].unsqueeze(0), values[idx].unsqueeze(0),
                                                   ...
                                                   torch.cat([queries[idx].to(device),responses[idx].to(device)]).unsqueeze(0))
    ...
    def batched_forward_pass(self, queries, responses,device, tokenizer):
        ...
        for i in range(int(bs/fbs)):
            ...
            # input_ids = self.data_collator([torch.cat((q.to(device), r.to(device)),dim=0) for q, r in zip(query_batch, response_batch)])["input_ids"]
            # input_ids = self.data_collator([{"input_ids": torch.cat((q.to(device), r.to(
            #     device)), dim=0)} for q, r in zip(query_batch, response_batch)])["input_ids"]
            input_ids = torch.nn.utils.rnn.pad_sequence(
                [torch.cat((q.to(device), r.to(device)),dim=0) for q, r in zip(query_batch, response_batch)], batch_first=True)


            with torch.no_grad():
                # input_ids = input_ids.cpu().numpy() 
                # if (input_ids >= self.model.config.vocab_size).any():
                #     raise ValueError("Input includes index out of range")

                logits, _, v = self.model(input_ids.to(device))                                    # logits -> (batch, max_output_len, vocab_size); v -> (batch, max_ouput_len)
